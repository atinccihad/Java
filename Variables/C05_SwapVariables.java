package day04_IncrementDecrement;

public class C02_SvapVariables {

	public static void main(String[] args) {
	
		/*
		 * 1-Verilen sayi1 ve sayi2 variable'larinin degerlerini degistiren (SWAP)bir program yaziniz Orn : sayi1=10 ve sayi2=20;
		 * kodcalýstýktan sonra sayi1=20 ve sayi2=10
		 */
		
		int sayi1=10;
		int sayi2=20;
		
		//baslangýcta sayi1=10 ve sayi2=20
		
		System.out.println("baslangicta sayi1=" + sayi1 + " ve sayi2=" + sayi2);
		
		//ilk once bos bir variable olusturalim
		@SuppressWarnings("unused")
		int bos;
		//sayi2 yi bos variable'a atayalim
		bos=sayi2;
		//sayi2 variable'ina sayi1 degerini degerinin atayalim
		sayi2=sayi1;
		//bos variable'daki degeri sayi 1'e atayalim
		
		System.out.println("sonucta sayi1=" + sayi1 + "ve sayi2=" + sayi2);
		
		
		
		
		System.out.println("sonucta sayi1=" + sayi1 + "ve sayi2=" + sayi2);
		
		
	}

}
